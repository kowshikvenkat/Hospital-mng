...
$con = new mysqli('localhost','root','','hospital_login1');
$sqll = "select * from users where email = '$email'";
$resulting=mysqli_query($con,$sqll); 
            if(!$resulting->num_rows > 0){
		if (!$result->num_rows > 0) {
			$sql = "INSERT INTO users (username, email, password,uniqueId)
			...
			if ($result) {
				...
				This is Your UNIQUEID:$uniqueId  
				Caution : Do not share your Id with anyone ";
		...
		echo "<script>alert('Woops! You have already enrolled in billing system')</script>";
	}
